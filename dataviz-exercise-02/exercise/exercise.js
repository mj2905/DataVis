...
console.log([1,2,3,4,5,6].filter(divisibleBy(3)))
...
function randomArray(N, min_val = 0, max_val = 100) {
  return range(N).map(x => randomInRange(min_val, max_val))
}
...
function normalizeCounts(obj) {
  const len = Object.values(obj).reduce((acc, val) => acc+val, 0)
  return Object.assign(...Object.entries(obj).map(([key, val]) => ({[key]: val/len})))
}
...
function simulateBall(v0, angle, num_steps = 256, dt = 0.05, g = -9.81) {
  ...
  const vx = v0 * Math.cos(DEG_TO_RAD * angle)
  const vy = v0 * Math.sin(DEG_TO_RAD * angle)
  const inc = increment(0)(dt)

  return range(num_steps)
  .map(_ => inc())
  .map(t => [vx * t, vy * t + (g * t * t * 0.5)])
